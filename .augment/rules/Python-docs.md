...
While Smalltalk methods are in separate files with the class name, the class schema is defined in `smalltalk/classes/_PythonClasses.gs`. Do not do subclass creation in any other file. In the same way, when writing code for AST nodes, put the methods in the existing files, but keep the class definitions in `smalltalk/ast/_PythonAst.gs`.

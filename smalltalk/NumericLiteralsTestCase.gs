...
testDecimalInteger
	...
	self
		...
		assert: num = 'int with: 1';

		assert: (expr := self statementsAt: 2) notNil;
		assert: expr.value.value = 'int with: 1234';

		assert: (expr := self statementsAt: 3) notNil;
		assert: expr.value.value = 'int with: 12345';

		assert: (expr := self statementsAt: 4) notNil;
		assert: expr.value.value = 'int with: 0';
...
testExponentFloat
	...
	self
		...
		assert: x.value.value = 'float with: 2000.0';

		assert: (x := self statementsAt: 31) notNil;
		assert: x.value.value = 'float with: 11.0';

		assert: (x := self statementsAt: 32) notNil;
		assert: x.value.value = 'float with: 132.0';

		assert: (x := self statementsAt: 33) notNil;
		assert: x.value.value = 'float with: 1.29e-05';

		assert: (x := self statementsAt: 34) notNil;
		assert: x.value.value = 'float with: 31400000000.0';
		yourself.
%
category: 'other'
method: NumericLiteralsTestCase
testFloatNumber
	...
	self
		...
		assert: x.value.value = 'float with: 3.14';

		assert: (x := self statementsAt: 26) notNil;
		assert: x.value.value = 'float with: 0.11';

		assert: (x := self statementsAt: 27) notNil;
		assert: x.value.value = 'float with: 13.2';

		assert: (x := self statementsAt: 28) notNil;
		assert: x.value.value = 'float with: 0.129';

		assert: (x := self statementsAt: 29) notNil;
		assert: x.value.value = 'float with: 22.41';
		yourself.
%
category: 'other'
method: NumericLiteralsTestCase
testHexadecimalInteger
	...
	self
		...
		assert: x.value.value = 'int with: 257';

		assert: (x := self statementsAt: 19) notNil;
		assert: x.value.value = 'int with: 10';

		assert: (x := self statementsAt: 20) notNil;
		assert: x.value.value = 'int with: 169';

		assert: (x := self statementsAt: 21) notNil;
		assert: x.value.value = 'int with: 33';

		assert: (x := self statementsAt: 22) notNil;
		assert: x.value.value = 'int with: 144';

		assert: (x := self statementsAt: 23) notNil;
		assert: x.value.value = 'int with: 251';
		yourself.
%
category: 'other'
method: NumericLiteralsTestCase
...
testOctalInteger
	...
	self
		...
		assert: x.value.value = 'int with: 7';

		assert: (x := self statementsAt: 13)  notNil;
		assert: x.value.value = 'int with: 1536';

		assert: (x := self statementsAt: 14)  notNil;
		assert: x.value.value = 'int with: 17';

		assert: (x := self statementsAt: 15)  notNil;
		assert: x.value.value = 'int with: 161';
		yourself.
%

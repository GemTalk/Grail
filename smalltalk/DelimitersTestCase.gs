...
testCommaTuple
	...
	self 
		...
		assert: ((x.elts at: 1) isKindOf: ConstantAst);
		assert: (y := x.elts at: 1) notNil;
		assert: y.value = 'int with: 4';
		assert: ((x.elts at: 2) isKindOf: ConstantAst);
		assert: (y := x.elts at: 2) notNil;
		assert: y.value = 'int with: 5';
		assert: ((x.elts at: 3) isKindOf: ConstantAst);
		assert: (y := x.elts at: 3) notNil;
		assert: y.value = 'int with: 6';
		yourself.
%
category: 'other'
method: DelimitersTestCase
...
testSliceList
	...
	self 
		...
		assert: (x.slice.lower.value = 'int with: 0');
		assert: (x.slice.upper isKindOf: ConstantAst);
		assert: (x.slice.upper.value = 'int with: 1');
...
testSliceListEmpty
	...
	self 
		...
		assert: (x.value.elts size == 3);
		assert: ((y := x.value.elts at: 1) isKindOf: ConstantAst);
		assert: y.value = 'int with: 7';
		assert: ((y := x.value.elts at: 2) isKindOf: ConstantAst);
		assert: y.value = 'int with: 8';
		assert: ((y := x.value.elts at: 3) isKindOf: ConstantAst);
		assert: y.value = 'int with: 9';
		assert: (x.slice isKindOf: SliceAst);
...
testSliceTuple
	...
	self 
		...
		assert: (x.slice.step.value = 'int with: 2');
		yourself.
%
category: 'other'
method: DelimitersTestCase
testSliceTupleFilled
	...
	self 
		...
		assert: (x.value.elts size == 3);
		assert: ((y := x.value.elts at: 1) isKindOf: ConstantAst);
		assert: y.value = 'int with: 10';
		assert: ((y := x.value.elts at: 2) isKindOf: ConstantAst);
		assert: y.value = 'int with: 11';
		assert: ((y := x.value.elts at: 3) isKindOf: ConstantAst);
		assert: y.value = 'int with: 12';
		assert: (x.slice isKindOf: SliceAst);
		assert: (x.slice.lower isKindOf: ConstantAst);
		assert: (x.slice.lower.value = 'int with: 1');
		assert: (x.slice.upper isKindOf: ConstantAst);
		assert: (x.slice.upper.value = 'int with: 2');
		assert: (x.slice.step isKindOf: ConstantAst);
		assert: (x.slice.step.value = 'int with: 3');
		yourself.
%

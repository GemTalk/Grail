...
match_group(PyObject *op, PyObject* args)
{
    ...
    switch (size) {
    case 0:
        result = match_getslice(self, PyLong_FromLong(0), Py_None);
        break;
    case 1:
...
expand_template(TemplateObject *self, MatchObject *match)
{
     ...
     * otherwise use a dynamically allocated array. */
    PyObject *buffer[10];
    PyObject **out = buffer;
    PyObject **dynbuf = NULL;
    if (self->chunks > (int)Py_ARRAY_LENGTH(buffer)) {
        dynbuf = (PyObject **)PyMem_Malloc(self->chunks * sizeof(PyObject *));
        if (!dynbuf) {
            PyErr_NoMemory();
            return NULL;
        }
    ...
    if (PyUnicode_Check(self->literal)) {
        ...
        result = _PyUnicode_JoinArray(empty_str, out, count);
        Py_DECREF(empty_str);
    }
    else {
        /* Build a list from out[] for PyBytes_Join */
        PyObject *list = PyList_New(count);
        if (list) {
            for (Py_ssize_t i = 0; i < count; i++) {
                PyList_SET_ITEM(list, i, out[i]);
            }
            PyObject *empty_bytes = PyBytes_FromStringAndSize("", 0);
            result = PyBytes_Join(empty_bytes, list);
            Py_DECREF(empty_bytes);
            /* List took ownership of items via SET_ITEM, so don't decref them */
            Py_DECREF(list);
            PyMem_Free(dynbuf);
            return result;
        }
...
#ifdef GRAIL_SHIM
/* ====================================================================
 * Hand-written argument parsing wrappers (replacing clinic/sre.c.h)
 *
 * The clinic file generates argument-parsing wrappers that use internal
 * CPython APIs (_PyArg_*). We replace them with hand-written versions
 * that use only public API.
 *
 * Common patterns:
 *   (cls, /, string, pos=0, endpos=PY_SSIZE_T_MAX) — METH_METHOD|METH_FASTCALL|METH_KEYWORDS
 *   (string, pos=0, endpos=PY_SSIZE_T_MAX) — METH_FASTCALL
 *   () — METH_NOARGS
 *   (memo) — METH_O
 * ==================================================================== */
...
/* Helper: parse (string, pos=0, endpos=MAX) from FASTCALL args */
static int
_sre_parse_string_pos_endpos(PyObject *const *args, Py_ssize_t nargs,
                              PyObject **string, Py_ssize_t *pos,
                              Py_ssize_t *endpos)
{
    if (nargs < 1 || nargs > 3) {
        PyErr_Format(PyExc_TypeError,
            "expected 1 to 3 arguments, got %zd", nargs);
        return -1;
    }
    *string = args[0];
    *pos = (nargs > 1) ? PyLong_AsSsize_t(args[1]) : 0;
    *endpos = (nargs > 2) ? PyLong_AsSsize_t(args[2]) : PY_SSIZE_T_MAX;
    return 0;
}
...
{
    return PyLong_FromLong(_sre_getcodesize_impl(module));
}

#define _SRE_GETCODESIZE_METHODDEF \
    {"getcodesize", _sre_getcodesize, METH_NOARGS, NULL},

/* _sre.ascii_iscased(character) */
static PyObject *
_sre_ascii_iscased(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1) {
        PyErr_Format(PyExc_TypeError, "expected 1 argument, got %zd", nargs);
        return NULL;
    }
    int character = (int)PyLong_AsLong(args[0]);
    if (character == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromLong(_sre_ascii_iscased_impl(module, character));
}

#define _SRE_ASCII_ISCASED_METHODDEF \
    {"ascii_iscased", (PyCFunction)(void*)_sre_ascii_iscased, METH_FASTCALL, NULL},

/* _sre.unicode_iscased(character) */
static PyObject *
_sre_unicode_iscased(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1) {
        PyErr_Format(PyExc_TypeError, "expected 1 argument, got %zd", nargs);
        return NULL;
    ...
    int character = (int)PyLong_AsLong(args[0]);
    if (character == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromLong(_sre_unicode_iscased_impl(module, character));
}

#define _SRE_UNICODE_ISCASED_METHODDEF \
    {"unicode_iscased", (PyCFunction)(void*)_sre_unicode_iscased, METH_FASTCALL, NULL},

/* _sre.ascii_tolower(character) */
static PyObject *
_sre_ascii_tolower(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1) {
        PyErr_Format(PyExc_TypeError, "expected 1 argument, got %zd", nargs);
        return NULL;
    ...
    int character = (int)PyLong_AsLong(args[0]);
    if (character == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromLong(_sre_ascii_tolower_impl(module, character));
}

#define _SRE_ASCII_TOLOWER_METHODDEF \
    {"ascii_tolower", (PyCFunction)(void*)_sre_ascii_tolower, METH_FASTCALL, NULL},

/* _sre.unicode_tolower(character) */
static PyObject *
_sre_unicode_tolower(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1) {
        PyErr_Format(PyExc_TypeError, "expected 1 argument, got %zd", nargs);
        return NULL;
    ...
    int character = (int)PyLong_AsLong(args[0]);
    if (character == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromLong(_sre_unicode_tolower_impl(module, character));
}

#define _SRE_UNICODE_TOLOWER_METHODDEF \
    {"unicode_tolower", (PyCFunction)(void*)_sre_unicode_tolower, METH_FASTCALL, NULL},

/* _sre.compile(pattern, flags, code, groups, groupindex, indexgroup) */
static PyObject *
_sre_compile(PyObject *module, PyObject *const *args, Py_ssize_t nargs,
             PyObject *kwnames)
{
    (void)kwnames;
    if (nargs != 6) {
        PyErr_Format(PyExc_TypeError,
            "compile expected 6 arguments, got %zd", nargs);
        return NULL;
    }
    ...
    Py_ssize_t groups = PyLong_AsSsize_t(args[3]);
    if (groups == -1 && PyErr_Occurred()) return NULL;
    PyObject *groupindex = args[4];
    PyObject *indexgroup = args[5];
    return _sre_compile_impl(module, pattern, flags, code, groups,
                              groupindex, indexgroup);
}

#define _SRE_COMPILE_METHODDEF \
    {"compile", (PyCFunction)(void*)_sre_compile, METH_FASTCALL|METH_KEYWORDS, NULL},

/* _sre.template(pattern, template) */
static PyObject *
_sre_template(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2) {
        PyErr_Format(PyExc_TypeError,
            "template expected 2 arguments, got %zd", nargs);
        return NULL;
    }
    return _sre_template_impl(module, args[0], args[1]);
}

#define _SRE_TEMPLATE_METHODDEF \
    {"template", (PyCFunction)(void*)_sre_template, METH_FASTCALL, NULL},

/* --- Pattern methods --- */

/* Pattern.match(cls, /, string, pos=0, endpos=MAX) */
static PyObject *
_sre_SRE_Pattern_match(PyObject *self, PyTypeObject *cls,
                       ...
                       PyObject *kwnames)
{
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_match_impl(_PatternObject_CAST(self), cls,
                                        string, pos, endpos);
}

#define _SRE_SRE_PATTERN_MATCH_METHODDEF \
    {"match", (PyCFunction)(void*)_sre_SRE_Pattern_match, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Pattern.fullmatch(cls, /, string, pos=0, endpos=MAX) */
static PyObject *
_sre_SRE_Pattern_fullmatch(PyObject *self, PyTypeObject *cls,
                           PyObject *const *args, Py_ssize_t nargs,
                           PyObject *kwnames)
{
    (void)kwnames;
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_fullmatch_impl(_PatternObject_CAST(self), cls,
                                            string, pos, endpos);
}

#define _SRE_SRE_PATTERN_FULLMATCH_METHODDEF \
    {"fullmatch", (PyCFunction)(void*)_sre_SRE_Pattern_fullmatch, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Pattern.search(cls, /, string, pos=0, endpos=MAX) */
static PyObject *
_sre_SRE_Pattern_search(PyObject *self, PyTypeObject *cls,
                        PyObject *const *args, Py_ssize_t nargs,
                        PyObject *kwnames)
{
    (void)kwnames;
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_search_impl(_PatternObject_CAST(self), cls,
                                         string, pos, endpos);
}
...
_sre_SRE_Pattern_findall(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_findall_impl(_PatternObject_CAST(self),
                                          string, pos, endpos);
}

#define _SRE_SRE_PATTERN_FINDALL_METHODDEF \
    {"findall", (PyCFunction)(void*)_sre_SRE_Pattern_findall, METH_FASTCALL, NULL},

/* Pattern.finditer(cls, /, string, pos=0, endpos=MAX) */
static PyObject *
_sre_SRE_Pattern_finditer(PyObject *self, PyTypeObject *cls,
                          PyObject *const *args, Py_ssize_t nargs,
                          PyObject *kwnames)
{
    (void)kwnames;
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_finditer_impl(_PatternObject_CAST(self), cls,
                                           string, pos, endpos);
}

#define _SRE_SRE_PATTERN_FINDITER_METHODDEF \
    {"finditer", (PyCFunction)(void*)_sre_SRE_Pattern_finditer, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Pattern.scanner(cls, /, string, pos=0, endpos=MAX) */
static PyObject *
_sre_SRE_Pattern_scanner(PyObject *self, PyTypeObject *cls,
                         PyObject *const *args, Py_ssize_t nargs,
                         PyObject *kwnames)
{
    (void)kwnames;
    ...
    if (_sre_parse_string_pos_endpos(args, nargs, &string, &pos, &endpos) < 0)
        return NULL;
    return _sre_SRE_Pattern_scanner_impl(_PatternObject_CAST(self), cls,
                                          string, pos, endpos);
}

#define _SRE_SRE_PATTERN_SCANNER_METHODDEF \
    {"scanner", (PyCFunction)(void*)_sre_SRE_Pattern_scanner, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Pattern.split(string, maxsplit=0) — METH_FASTCALL */
static PyObject *
_sre_SRE_Pattern_split(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs < 1 || nargs > 2) {
        PyErr_Format(PyExc_TypeError,
            "split expected 1 to 2 arguments, got %zd", nargs);
        return NULL;
    }
    PyObject *string = args[0];
    Py_ssize_t maxsplit = (nargs > 1) ? PyLong_AsSsize_t(args[1]) : 0;
    return _sre_SRE_Pattern_split_impl(_PatternObject_CAST(self),
                                        string, maxsplit);
}

#define _SRE_SRE_PATTERN_SPLIT_METHODDEF \
    {"split", (PyCFunction)(void*)_sre_SRE_Pattern_split, METH_FASTCALL, NULL},

/* Pattern.sub(cls, /, repl, string, count=0) */
static PyObject *
_sre_SRE_Pattern_sub(PyObject *self, PyTypeObject *cls,
                     PyObject *const *args, Py_ssize_t nargs,
                     PyObject *kwnames)
{
    (void)kwnames;
    if (nargs < 2 || nargs > 3) {
        PyErr_Format(PyExc_TypeError,
            "sub expected 2 to 3 arguments, got %zd", nargs);
        return NULL;
    }
    PyObject *repl = args[0];
    PyObject *string = args[1];
    Py_ssize_t count = (nargs > 2) ? PyLong_AsSsize_t(args[2]) : 0;
    return _sre_SRE_Pattern_sub_impl(_PatternObject_CAST(self), cls,
                                      repl, string, count);
}

#define _SRE_SRE_PATTERN_SUB_METHODDEF \
    {"sub", (PyCFunction)(void*)_sre_SRE_Pattern_sub, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Pattern.subn(cls, /, repl, string, count=0) */
static PyObject *
_sre_SRE_Pattern_subn(PyObject *self, PyTypeObject *cls,
                      PyObject *const *args, Py_ssize_t nargs,
                      PyObject *kwnames)
{
    (void)kwnames;
    if (nargs < 2 || nargs > 3) {
        PyErr_Format(PyExc_TypeError,
            "subn expected 2 to 3 arguments, got %zd", nargs);
        return NULL;
    }
    PyObject *repl = args[0];
    PyObject *string = args[1];
    Py_ssize_t count = (nargs > 2) ? PyLong_AsSsize_t(args[2]) : 0;
    return _sre_SRE_Pattern_subn_impl(_PatternObject_CAST(self), cls,
                                       repl, string, count);
}

#define _SRE_SRE_PATTERN_SUBN_METHODDEF \
    {"subn", (PyCFunction)(void*)_sre_SRE_Pattern_subn, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},
...
{
    return _sre_SRE_Pattern___copy___impl(_PatternObject_CAST(self));
}

#define _SRE_SRE_PATTERN___COPY___METHODDEF \
    {"__copy__", _sre_SRE_Pattern___copy__, METH_NOARGS, NULL},

/* Pattern.__deepcopy__(memo) */
#define _SRE_SRE_PATTERN___DEEPCOPY___METHODDEF \
    {"__deepcopy__", (PyCFunction)(void*)_sre_SRE_Pattern___deepcopy__, METH_O, NULL},

/* Pattern._fail_after(count, exception) — debug only, no-op in Grail */
#ifdef Py_DEBUG
static PyObject *
...
#define _SRE_SRE_PATTERN__FAIL_AFTER_METHODDEF \
    {"_fail_after", (PyCFunction)(void*)_sre_SRE_Pattern__fail_after, METH_FASTCALL, NULL},
#else
#define _SRE_SRE_PATTERN__FAIL_AFTER_METHODDEF
#endif
...
{
    PyObject *group = (nargs > 0) ? args[0] : PyLong_FromLong(0);
    Py_ssize_t result = _sre_SRE_Match_start_impl(_MatchObject_CAST(self), group);
    if (result == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromSsize_t(result);
}

#define _SRE_SRE_MATCH_START_METHODDEF \
    {"start", (PyCFunction)(void*)_sre_SRE_Match_start, METH_FASTCALL, NULL},
...
{
    PyObject *group = (nargs > 0) ? args[0] : PyLong_FromLong(0);
    Py_ssize_t result = _sre_SRE_Match_end_impl(_MatchObject_CAST(self), group);
    if (result == -1 && PyErr_Occurred()) return NULL;
    return PyLong_FromSsize_t(result);
}

#define _SRE_SRE_MATCH_END_METHODDEF \
    {"end", (PyCFunction)(void*)_sre_SRE_Match_end, METH_FASTCALL, NULL},

/* Match.span(group=0) -> (start, end) */
static PyObject *
_sre_SRE_Match_span(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    PyObject *group = (nargs > 0) ? args[0] : PyLong_FromLong(0);
    return _sre_SRE_Match_span_impl(_MatchObject_CAST(self), group);
}

#define _SRE_SRE_MATCH_SPAN_METHODDEF \
    {"span", (PyCFunction)(void*)_sre_SRE_Match_span, METH_FASTCALL, NULL},

/* Match.groups(default=None) */
static PyObject *
_sre_SRE_Match_groups(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    PyObject *default_value = (nargs > 0) ? args[0] : Py_None;
    return _sre_SRE_Match_groups_impl(_MatchObject_CAST(self), default_value);
}

#define _SRE_SRE_MATCH_GROUPS_METHODDEF \
    {"groups", (PyCFunction)(void*)_sre_SRE_Match_groups, METH_FASTCALL, NULL},

/* Match.groupdict(default=None) */
static PyObject *
_sre_SRE_Match_groupdict(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    PyObject *default_value = (nargs > 0) ? args[0] : Py_None;
    return _sre_SRE_Match_groupdict_impl(_MatchObject_CAST(self), default_value);
}

#define _SRE_SRE_MATCH_GROUPDICT_METHODDEF \
    {"groupdict", (PyCFunction)(void*)_sre_SRE_Match_groupdict, METH_FASTCALL, NULL},

/* Match.expand(template) */
#define _SRE_SRE_MATCH_EXPAND_METHODDEF \
    {"expand", (PyCFunction)(void*)_sre_SRE_Match_expand_impl, METH_O, NULL},

/* Match.__copy__() */
static PyObject *
_sre_SRE_Match___copy__(PyObject *self, PyObject *Py_UNUSED(ignored))
{
    return _sre_SRE_Match___copy___impl(_MatchObject_CAST(self));
}

#define _SRE_SRE_MATCH___COPY___METHODDEF \
    {"__copy__", _sre_SRE_Match___copy__, METH_NOARGS, NULL},

/* Match.__deepcopy__(memo) */
#define _SRE_SRE_MATCH___DEEPCOPY___METHODDEF \
    {"__deepcopy__", (PyCFunction)(void*)_sre_SRE_Match___deepcopy__, METH_O, NULL},

/* --- Scanner methods --- */

/* Scanner.match(cls) */
static PyObject *
_sre_SRE_Scanner_match(PyObject *self, PyTypeObject *cls,
                       PyObject *const *args, Py_ssize_t nargs,
                       PyObject *kwnames)
{
    (void)args; (void)nargs; (void)kwnames;
    return _sre_SRE_Scanner_match_impl(_ScannerObject_CAST(self), cls);
}

#define _SRE_SRE_SCANNER_MATCH_METHODDEF \
    {"match", (PyCFunction)(void*)_sre_SRE_Scanner_match, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* Scanner.search(cls) */
static PyObject *
_sre_SRE_Scanner_search(PyObject *self, PyTypeObject *cls,
                        PyObject *const *args, Py_ssize_t nargs,
                        PyObject *kwnames)
{
    (void)args; (void)nargs; (void)kwnames;
    return _sre_SRE_Scanner_search_impl(_ScannerObject_CAST(self), cls);
}

#define _SRE_SRE_SCANNER_SEARCH_METHODDEF \
    {"search", (PyCFunction)(void*)_sre_SRE_Scanner_search, \
     METH_METHOD|METH_FASTCALL|METH_KEYWORDS, NULL},

/* ====================================================================
 * End of hand-written clinic replacements
 * ==================================================================== */

#else  /* !GRAIL_SHIM — use the real clinic-generated wrappers */

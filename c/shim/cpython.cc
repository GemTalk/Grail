...
/* ====================================================================
 * _PyArg_UnpackKeywords — clinic-generated argument parser
 *
 * In the Grail shim, kwnames is always NULL (we never pass keyword args).
 * Fast path: validate positional arg count and return args directly.
 * ==================================================================== */
extern "C" PyObject *const *
_PyArg_UnpackKeywords(PyObject *const *args, Py_ssize_t nargs,
                      ...
                      PyObject **buf)
{
    (void)kwargs; (void)kwnames; (void)buf; (void)minkw;

    /* Fast path: no keyword arguments */
    if (nargs < minpos) {
        PyErr_Format(PyExc_TypeError,
                     "%s expected at least %d argument%s, got %zd",
                     parser->fname, minpos, minpos == 1 ? "" : "s", nargs);
        return NULL;
    }
    if (!varpos && nargs > maxpos) {
        PyErr_Format(PyExc_TypeError,
                     "%s expected at most %d argument%s, got %zd",
                     parser->fname, maxpos, maxpos == 1 ? "" : "s", nargs);
        return NULL;
    }
    return args;
}
...
extern "C" PyObject *PyUnicode_Substring(PyObject *str, Py_ssize_t start,
                                         Py_ssize_t end) {
    /* Extract substring by getting UCS-4 data and delegating to
       GemStone's copyFrom:to: (1-based, inclusive). */
    /* Get UCS-4 data from the cached conversion */
    Py_UCS4 *data;
    Py_ssize_t length;
    if (get_ucs4_for_string(str, &data, &length) < 0)
        return NULL;
    ...
    /* Convert UCS-4 slice back to UTF-8 */
    Py_ssize_t slice_len = end - start;
    /* Max 4 bytes per UCS-4 char */
    char *utf8 = (char *)malloc((size_t)(slice_len * 4 + 1));
    ...
    *p = '\0';
    ...
    PyObject *result = PyUnicode_FromStringAndSize(utf8, (Py_ssize_t)(p - utf8));
    free(utf8);
    return result;
}
...
/* ====================================================================
 * shimCall6 — like shimCall but supports 6 args
 *
 * Arguments (8 OopType):
 *   modMethodOop — String: "module.method" (split on last '.')
 *   a1–a6        — SmallInteger: CByteArray memory addresses (0 for unused)
 *   flagsOop     — SmallInteger: bits 0–2 = nargs (0–6), bit 3 = returnCPtr
 * ==================================================================== */

static OopType shimCall6(OopType modMethodOop, OopType a1, OopType a2,
                          OopType a3, OopType a4, OopType a5,
                          OopType a6, OopType flagsOop)
{
    char modMethod[128], modName[64], methName[64];
    fetch_string(modMethodOop, modMethod, sizeof(modMethod));

    /* Split "module.method" on last '.' */
    char *lastDot = strrchr(modMethod, '.');
    if (!lastDot) {
        char msg[256];
        snprintf(msg, sizeof(msg), "shimCall6: expected 'module.method', got '%s'", modMethod);
        raise_error(msg);
        return OOP_NIL;
    }
    size_t modLen = (size_t)(lastDot - modMethod);
    if (modLen >= sizeof(modName)) modLen = sizeof(modName) - 1;
    memcpy(modName, modMethod, modLen);
    modName[modLen] = '\0';
    strncpy(methName, lastDot + 1, sizeof(methName) - 1);
    methName[sizeof(methName) - 1] = '\0';
    ...
    PyObject *pyArgs[6] = {
        ...
        (PyObject *)(intptr_t)GciOopToI64(a4),
        (PyObject *)(intptr_t)GciOopToI64(a5),
        (PyObject *)(intptr_t)GciOopToI64(a6),
    };
...
extern "C" void GciUserActionInit(void) {
    ...
    GCI_DECLARE_ACTION("shimCall6", shimCall6, 8);

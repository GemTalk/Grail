...
### FIXED: evaluated code can be named, so its frames are not all `<grail>` (2026-09-09)

The follow-up 9.55 left open. A traceback out of a *script* named its file,
because importlib sets `CallAst >> sourcePath` for the compile; a traceback out
of EVALUATED code could not, because `ModuleAst`'s evaluate entry points had no
filename parameter. So the REPL, and any embedder driving `evaluateSource:`,
reported every frame as `File "<grail>"` — the placeholder codegen stamps when
nothing named the source.

`ModuleAst class >> whileCompilingWithFilename:do:` is the seam, and each
evaluate/execute entry point gained a `filename:` spelling over it. Nil means
"no opinion" and leaves the enclosing compile's name alone, so nothing that does
not pass one changes.

Naming the module body was not enough on its own. A function defined by
evaluated code is compiled as a *block inside the doit's own method*: it has no
module class for `___pythonFileForClassOf___` to ask, and it is not the body
that carries the `___pyFile___` stamp, so its frames stayed `<grail>` while the
`<module>` frame above them said `named.py`.
`BaseException class >> ___pythonFileForDoitOf___:` is the third and last
fallback, and it answers only for a doit somebody NAMED, reading a filename
`ModuleAst >> ___rememberDoitScope:for:` records at compile time in the registry
it already keeps and evicts. Answering `<grail>` for unnamed doits too would not
be equivalent: a frame with nothing of its own to say falls back to the
*catching* code object's filename, so answering at all stops it — which must not
happen for every `exec()` and `eval()` in the corpus.

The REPL passes `'<stdin>'`, which is what CPython's REPL reports, and the two
now agree line for line:
...
In the same change, `eval()` began honouring `compile()`'s second argument the
way `exec()` always has: `eval(compile(src, 'sums.py', 'eval'))` reported
`<grail>` where the same source through `exec()` reported `sums.py`.

### OPEN: `test_decimal`'s `test_abc` is install-time nondeterministic (2026-09-10)

`test.test_decimal`'s `PyPythonAPItests.test_abc` — `issubclass(_pydecimal.Decimal,
numbers.Number)` — passes or fails according to the `install.sh` the image was
built with, not according to the code under test. Byte-identical source read
`f=2` (test_abc failing) on one install and `f=1` on each of the next two;
`main` behaves the same way. The registration is `_numbers.Number.register(Decimal)`
at `_pydecimal.py:3831`, and the virtual-subclass registry is a module-level dict
in `abc.py` keyed by class identity, so a lost registration means either a second
`abc` instance or a rebuilt `numbers.Number` — presumably an ordering difference
in the canonical-module rebuild. Not diagnosed further.

**Two consequences worth knowing.** The nightly can report this row as a
regression or an improvement with nothing having changed. And two suite samples
taken within one install always agree, so a variant sampled twice without
reinstalling has been measured once — which invalidated four rounds of bisection
during the 9.56 work before it was noticed. Reinstall before believing a moved
row, and sample across installs.

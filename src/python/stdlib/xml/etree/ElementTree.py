# GRAIL: CPython 3.14.7's xml/etree/ElementTree.py, VERBATIM -- replacing a
# 269-line hand-rolled shim that could BUILD and SERIALIZE a tree but not parse
# one (fromstring/parse raised NotImplementedError).
#
# The whole file is pure Python; only the `_elementtree` C accelerator is not,
# and its import at the bottom is already inside a try.  Parsing reaches
# `xml.parsers.expat`, which Grail now has -- so this needed no change at all.
#
# The old shim kept an explicit `_attr_order` list because, when it was
...
# delete-then-reinsert case, which is what let this file replace it.

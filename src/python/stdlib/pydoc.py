...
try:
    ...
    # GRAIL: these three are CPython's loader machinery, and Grail replaces the
    # import system wholesale rather than reimplementing them -- so they are
    # absent, and faking them would advertise an import model Grail does not
    # have.  Guarded rather than removed, because the code that reaches for
    # them is the code that documents a FILE ON DISK which has not been
    # imported: synopsis() of a compiled module, importfile(), and the module
    # scanner behind apropos() / ``pydoc -k''.  Documenting a live object --
    # which is all of help() -- touches none of it.  Each use site below says
...
def synopsis(filename, cache={}):
    ...
    if lastupdate is None or lastupdate < mtime:
        ...
        # GRAIL: with no loader machinery there is no binary branch to take, so
        # every file is treated as source.  A genuinely compiled module then
        # fails to tokenize and answers None, which is what this function
        # already promises for a file it cannot read.
        if not _LOADERS_AVAILABLE:
            loader_cls = None
        elif filename.endswith(tuple(importlib.machinery.BYTECODE_SUFFIXES)):
...
def importfile(path):
    ...
    # GRAIL: importing a file BY PATH is the loader machinery's job, and Grail
    # has its own importer rather than CPython's.  Refused by name here instead
    # of failing later as a NameError, so a caller is told which capability is
    # missing.
    if not _LOADERS_AVAILABLE:
        raise ErrorDuringImport(path, ImportError(
            'pydoc.importfile needs CPython loader machinery '
            '(importlib._bootstrap_external), which Grail does not provide'))
        ...
        for importer, modname, ispkg in pkgutil.walk_packages(onerror=onerror):
            ...
            if key is None:
                ...
                if hasattr(loader, 'get_source'):
                    ...
                    # GRAIL: a module with no source to read can only be
                    # described by loading it, which needs the loader
                    # machinery.  Skipped like any module that fails to load,
                    # so a scan still reports every module it CAN describe.
                    if not _LOADERS_AVAILABLE:
                        if onerror:
                            onerror(modname)
                        continue

...
if kwargs:
    ...
    q = params = ""
"""
# Grail-patched: ast.parse() isn't implemented yet; the AST nodes
# below feed into a runtime code-generation path for URL building.
# Routes still MATCH (the matcher doesn't use these), but
# url_for() / Rule.build() that need this AST-based generator will
# raise at first use.  Set to None so module-init completes.
_IF_KWARGS_URL_ENCODE_AST = None
_URL_ENCODE_AST_NAMES = (None, None)

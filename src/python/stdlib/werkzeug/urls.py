...
def _make_unquote_part(name: str, chars: str) -> t.Callable[[str], str]:
    ...
    # Grail-patched: ExecBlock doesn't support dynamic-instVar
    # storage, so setattr on a closure (``fn.__name__ = ...'')
    # raises ImproperOperation.  The __name__ tag is only for
    # introspection / repr; safe to skip in this shim.

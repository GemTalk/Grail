...
#
# GRAIL: upstream CPython 3.14.6, VERBATIM.  It binds no codec entry point into
# a class body (see encodings/utf_8.py), so it needed no edit.  It replaces
# the ASCII-only 'idna' that str.encode / bytes.decode used to answer for
# themselves, which could not decode an 'xn--' label or encode a non-ASCII one.

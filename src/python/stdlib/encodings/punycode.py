...
#
# GRAIL: upstream CPython 3.14.7, VERBATIM.  Nothing here binds a codec
# entry point straight into a class body, which is the one edit modules in
# this package usually need (see latin_1.py) -- punycode's Codec calls its
# module-level functions, so it ports unchanged.
#
# It is an ALGORITHM rather than a table, which is why it could be shipped
# when ``idna``, the other name in this family, could not: idna needs the
# whole stringprep module and unicodedata.ucd_3_2_0, a Unicode database
# pinned at version 3.2 that Grail's five-function unicodedata shim could
# only fake.  See tests/python/more_codecs.py.

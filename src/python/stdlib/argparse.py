...
# epilog, add_help.  The other eleven raised TypeError on an unexpected
# keyword argument, which is how the kaggle CLI failed to start: kaggle.cli
# passes formatter_class=RawTextHelpFormatter.  The three formatter classes it
# names all EXISTED in the subset, as empty pass-bodied stubs the constructor
# would not take.
#
# WHY THE SOURCE DROP AND NOT ELEVEN MORE PARAMETERS.  The missing eleven are
# not eleven independent switches; each one is a thread through the same
# fabric.  formatter_class means a real help LAYOUT engine (usage line
# wrapping, action-invocation columns, %(default)s expansion); parents means
# action objects that can be shared between parsers, which means the registry
# and the container/group split; conflict_handler means the option-string
# index has to support removal.  And the parameters are not even the whole
# gap: the subset had no subparsers at all -- ``kaggle datasets list'' is a
# subcommand -- and _SubParsersAction was a bare ``pass''.  A hand-written
# version of all that is a reimplementation of this file minus the parts that
# happened not to be exercised yet, which is the position the subset was
# already in.
#
...
#
#   * ClassDefAst: a class NESTED in another class body had its method bodies
#     generated under the OUTER class's class-body flags, so with a
#     locals()/vars() call anywhere in the outer class every name read in the
#     nested class's methods emitted a probe whose receiver was the nested
#     class's Smalltalk BLOCK TEMP -- invisible to the runtime compileMethod:.
#     Those methods did not compile at all and became codegen-gap stubs.
#     HelpFormatter._Section and _SubParsersAction._ChoicesPseudoAction are
#     this file's two instances; HelpFormatter._expand_help's ``vars(action)''
#     is what turned the flag on.
#   * str >> _splitlines:kw: did not exist, so the KEYWORD spelling
#     ``text.splitlines(keepends=True)'' -- RawDescriptionHelpFormatter's
#     _fill_text -- raised TypeError.  bytes had had the varargs entry all
#     along; str had only the 0- and 1-argument forms.
#   * shutil.get_terminal_size and _colorize's argparse theme section +
#     decolor() were missing; HelpFormatter.__init__ calls both.
#
# WHAT IS NOT IMPLEMENTED: nothing in this file.  All 15 constructor
# parameters, subparsers, argument groups, mutually exclusive groups,
# fromfile_prefix_chars, allow_abbrev, exit_on_error, suggest_on_error and
# color=True all work.  The one deviation is inherited from shutil: with no
# ioctl, get_terminal_size answers $COLUMNS or the 80x24 fallback rather than
# the real terminal width, so help text wraps at 78 columns unless COLUMNS is
# set.  CPython on a build without the terminal query does the same.
#

...
# GRAIL: CPython uses ``class _NamedIntConstant(int):`` here so that
# opcode reprs show names like ``LITERAL`` rather than ``16``.  Grail
# can't subclass int with extra inst vars (SmallInteger has no slots),
# so we substitute the built-in NamedIntConstant wrapper class, which
...
# GRAIL: ``del OPCODES[-2:]`` upstream — rewritten as a slice
# rebind because Grail doesn't yet support ``del`` on subscript
# targets with a slice index (see TODO.md > "del statement panics
# codegen" and "Slice subscript assignment / del").  Both MIN_REPEAT
# and MAX_REPEAT remain as module-level names (the parser uses them).
OPCODES = OPCODES[:-2]

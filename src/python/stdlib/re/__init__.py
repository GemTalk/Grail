...
# .sub()/.subn()/.split().  Grail can't subclass int (SmallInteger
# has no inst-var slots).  Use a plain marker object and coerce to
# 0 at each call site below — the sentinel never escapes the
# function bodies, so the int identity isn't actually needed.
class _ZeroSentinel:
    pass

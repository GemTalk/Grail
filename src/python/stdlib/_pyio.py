...
# GRAIL ADAPTATION: not ``from _thread import allocate_lock as Lock''.  Grail
# HAS a _thread module, so that import SUCCEEDS -- and binds a LOCK, not the
# callable: allocate_lock is a module METHOD, so importing the name READS the
# attribute, which performs the getter.  ``Lock()'' then raises "'PyThreadLock'
# object is not callable" from every buffered constructor.  Calling through the
# module instead takes the attribute-CALL path, which does reach the method and
# answers a fresh lock per call, so one forwarding def restores the name the
# rest of this file expects.  reprlib documents the same gap for _thread.get_ident.
import _thread
...
import io
from io import (SEEK_SET, SEEK_CUR, SEEK_END)  # PROBE

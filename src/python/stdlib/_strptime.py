...
def _thread_allocate_lock():
    # GRAIL: reach _thread through a LOCAL import so the name resolves to the
    # module instance (not the _thread class); a module-level
    # `from _thread import allocate_lock` alias binds a lock *instance* under
    # Grail's importer, which is then not callable.
    import _thread
...
class TimeRE(dict):
    ...
    def pattern(self, format):
        ...
        def repl(m):
            # GRAIL: rewritten from a match/case statement (PEP 634), which
            # Grail's codegen does not support, to equivalent if/elif.

...
# GRAIL: upstream writes ``class _JoinablePath(ABC):`` and gets ABCMeta -- and
# so the register() called at the bottom of this file -- from ABC.  Grail's ABC
# does not carry ABCMeta yet (abc.py says why), so the metaclass is named here:
# the spelling abc.py recommends, and one CPython accepts unchanged.
# _ReadablePath and _WritablePath inherit it.
class _JoinablePath(ABC, metaclass=ABCMeta):

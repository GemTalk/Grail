...
def load_tests(loader, tests, pattern):
    ...
    # CPython walks ``directory'' unconditionally here even though it has
    # already computed ``skip_expected'' from os.path.isdir(directory) -- in
    # CPython the directory always exists, so the listdir never fails.  Grail
    # deliberately does NOT vendor the decimaltestdata/ corpus (several MB of
    # .decTest files), so the directory is absent and the bare listdir raised
    # FileNotFoundError out of load_tests, which aborts the WHOLE module
    # before a single test runs.  Guarding on the flag the file already
    # maintains keeps every other test class in this file reachable; the
    # IBMTestCases classes are then built with no test_* methods and pass
    # vacuously, exactly as they do in CPython when the corpus is missing.

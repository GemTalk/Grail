...
class TestTracebackType(unittest.TestCase):
    ...
    def test_attrs(self):
        # Grail: ``self.assertIs(tb.tb_frame, sys._getframe())'' below is an
        # IDENTITY assertion between two observations of the SAME activation,
        # made at different moments by different mechanisms.  CPython has one
        # mutable frame object per activation -- allocated at call time, with
        # f_lineno tracking the instruction pointer as it runs -- so both
        # observations hand back that one object.  Grail has no such object: a
        # Python function is a Smalltalk method, its activation is a GemStone VM
        # stack frame, and the only way a running gem can read its own stack is
        # the raise-time capture of (method, ip, receiver) triples (see
        # BaseException class>>___liveFrameChain___).  Each request therefore
        # SYNTHESISES a fresh PyFrame with the line baked in, which is why the
        # failure reads ``<frame at ..., line 248> is not <frame at ..., line
        # 253>'' -- one activation, seen twice.  Measured, not inferred: under
        # Grail ``sys._getframe() is sys._getframe()'' is False within a single
        # activation, and a frame re-read later still reports its capture line
        # where CPython's reports the current one.
        #
        # On-demand interning does not rescue it: ``ip'' is precisely what
        # differs between the two observations, so it cannot be part of a key,
        # and (method, receiver) is not unique under recursion.  Closing this
        # needs first-class activations -- a PyFrame allocated on every Python
        # call -- so it is an architectural item, not a traceback bug.
        #
        # The REST of this test is not vacuous (unlike test_3611): the tb_next
        # type guard, the deletion TypeError, both loop ValueErrors and the
        # valid assignments all pass under Grail.  None of that coverage is lost
        # by skipping here -- every one of those nine assertions has a peer in
        # tests/python/traceback_object.py (EXPECTED generated by running
        # CPython 3.14.6) and in PythonTests/TracebackObjectTestCase.
        self.skipTest("Grail: needs one mutable frame object per activation; "
                      "frames are synthesised per request from the VM stack "
                      "capture, so tb_frame is never identical to _getframe().")
        try:
            self.raiser()

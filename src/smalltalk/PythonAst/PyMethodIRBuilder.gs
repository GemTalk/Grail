...
Object subclass: 'PyMethodIRBuilder'
	instVarNames: #(methNode targetClass env curOffset locals sourceBase blockStack lexLevel loopStack handlerExStack genLeaf nestedFnDepth closureStack)
	classVars: #()
	classInstVars: #()
	poolDictionaries: #()
...
method: PyMethodIRBuilder
tempNamed: aSymbol
	"A method temp registered under aSymbol -- or, while a CLOSURE body is
	being emitted (nestedFunctionDo:, cut 64), a temp of the innermost closure
	block instead.  The emitters allocate their helpers lazily and reuse them
	by name (``___iter0___'', ``___item0___'', ``___unpack___'', ``___fn___''),
	which is sound within ONE frame; a nested def's block runs in the MIDDLE of
	the enclosing frame's statements -- its ``for'' inside the enclosing
	``for'' -- so a helper shared with the enclosing method would be clobbered
	mid-loop (EventLoopTestCase: the iterator of the enclosing async for
	became the closure's range_iterator).  The text declares those helpers as
	block temps per use; a per-closure temp is the same isolation."
	...
	closureStack isEmpty
		...
		ifFalse: [
			...
			leaf := (PyMethodIRBuilder node: #GsComVarLeaf) new
				blockTemp: aSymbol sourceLexLevel: (frame at: 2).

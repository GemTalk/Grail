...
**`CENSUS.md` IS regenerated now**, and the note that used to stand here --
that the corpus-2 scripts were not in the repository, so running
`census_report.py` would overwrite the corpus-2 section with nothing -- is
obsolete: `census_tests_00..02.tpz` landed with the census fix in #887, and the
board below is a full four-script run on the current tree.  Any matched-pair
subset measured for a single cut is still reported as such, but the committed
board no longer has to be reasoned around.

Of the stdlib's 1592 top-level defs **1571 (98.7%)** compile through IR; of its
4621 class-body methods **4544 (98.3%)** are built through the seam; of all 6417
defs the stdlib holds, **95.3%** go through IR.  Corpus 2 -- the CPython suite
manifest, which is the corpus that still has room -- reads 1299 of 1327
top-level defs (97.9%) and 8776 of 10942 class-body methods (80.2%), **81.1% of
all 12420 defs**.

**One row now dominates the whole remaining board.**  Of 2221 refusals across
both corpora, `method:classNotAtModuleScope` is **1746, or 78.6%** -- the
methods of a class defined inside a method, still compiled by
`<cls> ___compileMethod: '<source>'`.  Everything else together is 475, and the
largest single item in that tail is 77.  So the ranked roadmap below has one
entry that matters and a long tail; a cut anywhere in the tail moves the board
by tenths of a percent, and this one moves it by roughly fourteen points.

The `stmt:ClassDefAst` row is GONE -- cut 76 absorbed all 966 of it, which is
why the class-statement family now appears only as its refusal shapes.  The
biggest of those are class DECORATORS and class KEYWORDS (42 + 27), which are
one problem: both are expressions the class emit evaluates in the ENCLOSING
scope, so they need the marshalling the captures got in cuts 77-78, applied to
the statements around the class rather than inside it.

**Not worth doing: transcribing the class emit itself into IR.**  It is the
obvious-looking successor to cut 76 and it buys nothing measurable.  Cut 76
already routes every ELIGIBLE class statement through IR -- the statement the IR
method executes is a real IR send -- so replacing the compiled-text helper with
transcribed nodes would retire zero census rows.  What it would cost is a second
copy of `printSmalltalkRuntimeOn:`, ~2400 lines of branches, free to drift from
the text path that is the oracle for every other emit.  The current arrangement
gets the text path's sends BY CONSTRUCTION, because the Smalltalk compiler
produces them.  See `___irEligibleStatementLocals___:`, which argues this at the
site.
...
corpus-2 scripts are not in the repository, so `census_report.py` with corpus 1
alone would overwrite the corpus-2 section outright).

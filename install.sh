...
GRAIL_ENV1_RESTRICTED=$(LC_ALL=C topaz -lq -S "$GRAIL_DIR/scripts/detect_env1_restricted_classes.gs" 2>/dev/null \
    | grep -oE 'GRAIL_ENV1_RESTRICTED=(yes|no)' | head -1)
echo "Kernel capability: ${GRAIL_MODERN:-GRAIL_MODERN=? (probe failed -> legacy)}"
echo "                   ${GRAIL_ENV1_RESTRICTED:-GRAIL_ENV1_RESTRICTED=? (probe failed -> no)}"
# Must mirror install_base.sh's three tiers exactly -- whatever install_base.sh
# did NOT file as SystemUser has to be filed per-user here, and vice versa.  The
# two capabilities are independent: env-1 session methods on restricted classes
# governs the four env-1 extension files, while native wider-arity performMethod:
# governs Object_perform_allocators/primitives (a <primitive:> needs a privilege
# an ordinary user lacks).
{
    echo "! GENERATED by install.sh -- do not edit, not committed (out/ is gitignored)."
    echo "! Kernel-class extensions filed per-user, per detected kernel capability."
    echo "! Detected: ${GRAIL_MODERN:-unknown} ${GRAIL_ENV1_RESTRICTED:-unknown}"
    if [ "$GRAIL_MODERN" = "GRAIL_MODERN=yes" ] \
        || [ "$GRAIL_ENV1_RESTRICTED" = "GRAIL_ENV1_RESTRICTED=yes" ]; then
    ...
    if [ "$GRAIL_MODERN" = "GRAIL_MODERN=yes" ]; then
        ...
        echo "! Object_perform_allocators/primitives were filed as SystemUser (they need"
        echo "! <primitive:> compilation privileges, or an untested MR#6 per-env store)."
    fi
    if [ "$GRAIL_MODERN" != "GRAIL_MODERN=yes" ] \
        && [ "$GRAIL_ENV1_RESTRICTED" != "GRAIL_ENV1_RESTRICTED=yes" ]; then
        echo "! legacy kernel: all six were filed as SystemUser by install_base.gs."
    fi
} > "$GEN_INC"
